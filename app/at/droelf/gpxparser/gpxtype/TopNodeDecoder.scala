...
import at.droelf.gpxparser.extensions.track.{TrackExtensionDecoder, TrackStatsExtensionDecoder}
import java.io.File
...
object GPXDecoder extends DecoderUtils{
  ...
  def decodeFromFile(filePath : String): GPX = decodeFromFile(new File(filePath))

  def decodeFromFile(file: File): GPX = decode(loadFile(file))
